      ...
      <v-row>
        <v-col v-for="item in props.items" :key="item.name" cols="12" sm="6" md="4" lg="3">
          <v-card max-height="400">
            <v-card-title>
              <h4>{{ item.id }}</h4>
              <v-btn color="info" @click.stop="fn_order(item,'y')" dark large>승인</v-btn>
              <v-btn color="error" @click.stop="fn_order(item,'n')" dark large>거부</v-btn>
              <v-btn color="success" @click.stop="fn_order(item,'z')" dark large>제조완료</v-btn>
            </v-card-title>
            <v-divider></v-divider>
            <v-list dense>
