<template>
  <v-data-iterator :items="order.now"
hide-default-footer>
...
:key="item.name" cols="3">
          <v-card class="mx-auto"
max-width="344">
            <v-card-title>
              <template v-if="item.state == 1">
                <v-btn color="info"
dark large @click.stop="fnitem(item, 1)">
                  승인
                </v-btn>
                <v-btn color="error"
dark large @click.stop="fnitem(item, 0)">
                  거부
                </v-btn>
              </template>
              <template v-else-if="(item.state = 2)">
                <v-btn color="success"
dark large @click.stop="fnitem(item, 2)">
                  제조완료
                </v-btn>
              </template>
            </v-card-title>
            <v-card-text>
              <v-card
                v-for="(p_group, j) in item.products"
                :key="'_' + j"
                cols="1"
                raised
              >
                ...
                <v-card-text>
                  <v-list>
                    <v-list-tile-title v-for="p in p_group"
:key="p.name">
                      <v-chip-group column>
                        <v-chip
                          v-for="optg in p.option_group_list"
                          :key="optg.name"
                          :color="
